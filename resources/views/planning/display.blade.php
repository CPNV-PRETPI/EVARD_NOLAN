                ...
                @if($planning)
                    <div class="grid grid-cols-1 lg:grid-cols-7 gap-4 m-2">
                        @foreach($period as $date)
                            @foreach($planning->recipes as $recipe)
                                @if($date->isSameDay(Carbon\Carbon::parse($recipe->pivot->planned_for)))
                                    @foreach(['Déjeuner', 'Dîner', 'Souper'] as $moment)
                                        @if($recipe->pivot->moment_of_meal === $moment)
                                        <div class="flex flex-col gap-2 rounded-lg shadow-md lg:rounded-none lg:shadow-none bg-gray-200 lg:bg-white p-2 lg:p-0">
                                            <div class="text-xl">{{$date->format('l')}}</div>
                                            ...
                                            <div class="bg-gray-100 hover:bg-gray-300 rounded-lg border-2 border-gray-400 text-sm p-1">
                                                <div class="text-md">{{$recipe->title}}</div>
                                                <div class="">
                                                    <strong>Temps de préparation: </strong>{{$recipe->preparation_time}}<br>
                                                    <strong>Nombre de personnes: </strong>{{$recipe->nb_of_people}}
                                                </div>
                                            </div>
                                        </div>
                                        @else
                                            <div class="flex flex-col gap-2 rounded-lg shadow-md lg:rounded-none lg:shadow-none bg-gray-200 lg:bg-white p-2 lg:p-0">
                                                ...
                                                <div class="bg-white hover:bg-gray-300 rounded-lg border-2 border-dashed border-gray-400 text-sm p-1">
